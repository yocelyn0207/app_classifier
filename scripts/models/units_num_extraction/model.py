...
from scripts.models.units_num_extraction.dataset_preparation import LandstackDataset
...
class CustomT5(pl.LightningModule):
    def __init__(self, config, pretrained_T5_model_name: str,
    ...
    def _precision_recall_fscore(self, preds_ids, labels):
        # preds_ids: indices of tokens, e.g., tensor([[    1, 32098,     2,    46, 32098]]). (batch_size, target_max_token_len)
        # labels:

        # decode into strings rather than compare ids directly, because token 3 represents null, which is equal to mask token -100 in labels_ids
        preds = np.array(self.tokenizer.batch_decode(preds_ids, skip_special_tokens=True)) # (batch_size) with strings
        labels = np.array(labels)

        y_pred = preds==labels   # (batch_size) with true or false
        y_true = torch.tensor([True]*preds_ids.shape[0])

        # We don't calculate distance of y_pred and y_true, it should be either correct or not.
        # Recall, and f1-score are not applicable, because there is no False in y_true
        precision = precision_recall_fscore_support(y_true=y_true, y_pred=y_pred,average='micro')[0]
        return precision
    ...
    def validation_step(self, batch, batch_idx):
        ...
        return {'validate_loss': loss, 'preds': preds, 'preds_ids': preds_ids, 'labels_ids': output_ids,'labels':labels}
    ...
    def validation_epoch_end(self, outputs):
        ...
        precision = self._precision_recall_fscore(preds_ids, labels)
    ...
    def test_epoch_end(self, outputs):
        ...
        results = process_preds_to_readable_file(model = 'units_num_extraction', preds = preds,
    ...
    def prepare_data(self):
        if self.train_file_name is not None:
            ...
            self.train_dataset = LandstackDataset(train_DataFrame, self.tokenizer, self.source_max_token_len, self.target_max_token_len)
        ...
        if self.valid_file_name is not None:
            ...
            self.valid_dataset = LandstackDataset(valid_DataFrame, self.tokenizer, self.source_max_token_len, self.target_max_token_len)
        ...
        if self.test_file_name is not None:
            ...
            self.test_dataset = LandstackDataset(self.test_DataFrame, self.tokenizer, self.source_max_token_len, self.target_max_token_len)
        ...
        if self.predict_file_name is not None:
            ...
            self.predict_dataset = LandstackDataset(self.predict_DataFrame, self.tokenizer, self.source_max_token_len, self.target_max_token_len)
