...
def training_setup(config, num_epochs: int, gpus_per_trial: float, tpu_cores_ID:int,
                   pretrained_T5_model_name: str, use_classes: list,
                   ...
                   predict_file_name: str = None, predict_preds_save_file_name: str = 'predict_preds',
    ...
    if best_checkpoint_file_path_for_predict is None:
        model = CustomT5(config = config, pretrained_T5_model_name = pretrained_T5_model_name, use_classes = use_classes,
        ...
        model = CustomT5.load_from_checkpoint(checkpoint_path = best_checkpoint_file_path_for_predict,
                         config = config, pretrained_T5_model_name = pretrained_T5_model_name, use_classes = use_classes,
                         source_max_token_len = source_max_token_len,
                         num_beams = num_beams, num_workers = num_workers,
                         data_dir = data_dir, save_dir = save_dir,
                         train_file_name = train_file_name,
    ...
    results = process_preds_and_labels_to_readable_list(predict_DataFrame, preds, labels)
